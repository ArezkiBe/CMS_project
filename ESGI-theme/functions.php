...
function esgi_customize_register($wp_customize)
{
    $wp_customize->add_section('custom_image_section', array(
        'title' => __('Custom Image', 'your-theme-textdomain'),
        'description' => __('Upload an image to display on the front page', 'your-theme-textdomain'),
        'priority' => 30,
    ));
    ...
    $wp_customize->add_control(new WP_Customize_Image_Control($wp_customize, 'custom_image_control', array(
        'label' => __('Custom Image', 'your-theme-textdomain'),
    ...
    $wp_customize->add_control('has_footer_search', [
        ...
        'priority' => 1,
        'section' => 'esgi_section',
        'label' => __('Afficher la recherche dans le footer'),
    ]);

    // ajout des settings pour les URLs des réseaux sociaux
    ...
    foreach ($social_networks as $network) {
        ...
        // Ajout des controls pour les URLs des réseaux sociaux
        $wp_customize->add_control('url_' . $network, [
            'type' => 'url',
            'priority' => 1,
            'section' => 'esgi_section',
            'label' => sprintf(__('URL %s', 'nom-du-theme'), ucfirst($network)),
        ]);
    }
    ...
    for ($i = 1; $i <= 6; $i++) {
        ...
        $wp_customize->add_control(new WP_Customize_Image_Control($wp_customize, 'partner_logo_' . $i, array(
            ...
            'section' => 'title_tagline', // You can create a new section if needed
    ...
    $wp_customize->add_setting('service_bottom_image', array(
        'default' => '',
        'sanitize_callback' => 'esc_url_raw',
    ));

    $wp_customize->add_control(new WP_Customize_Image_Control($wp_customize, 'service_bottom_image', array(
        'label' => 'Bottom Image',
        'section' => 'title_tagline', // You can create a new section if needed
        'settings' => 'service_bottom_image',
    )));
    ...
    $wp_customize->add_control(new WP_Customize_Image_Control($wp_customize, 'about_us_image_control', array(
        'label' => __('About Us Image', 'your-theme-textdomain'),
        'section' => 'title_tagline',
        'settings' => 'about_us_image',
    )));
    
    // Option pour l'image collée à gauche
    $wp_customize->add_setting('side_image', array(
        'default' => '',
        'transport' => 'refresh',
    ));
    $wp_customize->add_control(new WP_Customize_Image_Control($wp_customize, 'side_image_control', array(
        'label' => __('Side Image', 'your-theme-textdomain'),
        'section' => 'title_tagline',
        'settings' => 'side_image',
    )));
    ...
    for ($i = 1; $i <= 3; $i++) {
        ...
        $wp_customize->add_setting('block_' . $i . '_description', array(
            'default' => '',
            'transport' => 'refresh',
        ));

        $wp_customize->add_control('block_' . $i . '_title', array(
            'label' => __('Block ' . $i . ' Title', 'your-theme-textdomain'),
            'section' => 'title_tagline',
            'type' => 'text',
        ));
        $wp_customize->add_control('block_' . $i . '_description', array(
            'label' => __('Block ' . $i . ' Description', 'your-theme-textdomain'),
            'section' => 'title_tagline',
            'type' => 'textarea',
        ));
    }
    ...
    for ($i = 1; $i <= 4; $i++) {
        ...
        $wp_customize->add_setting('team_' . $i . '_title', array(
            'default' => '',
            'transport' => 'refresh',
        ));
        $wp_customize->add_setting('team_' . $i . '_phone', array(
            'default' => '',
            'transport' => 'refresh',
        ));
        $wp_customize->add_setting('team_' . $i . '_email', array(
            'default' => '',
            'transport' => 'refresh',
        ));

        $wp_customize->add_control(new WP_Customize_Image_Control($wp_customize, 'team_' . $i . '_image_control', array(
            'label' => __('Team ' . $i . ' Image', 'your-theme-textdomain'),
            'section' => 'title_tagline',
            'settings' => 'team_' . $i . '_image',
        )));
        $wp_customize->add_control('team_' . $i . '_title', array(
            'label' => __('Team ' . $i . ' Title', 'your-theme-textdomain'),
            'section' => 'title_tagline',
            'type' => 'text',
        ));
        $wp_customize->add_control('team_' . $i . '_phone', array(
            'label' => __('Team ' . $i . ' Phone', 'your-theme-textdomain'),
            'section' => 'title_tagline',
            'type' => 'text',
        ));
        $wp_customize->add_control('team_' . $i . '_email', array(
            'label' => __('Team ' . $i . ' Email', 'your-theme-textdomain'),
            'section' => 'title_tagline',
            'type' => 'text',
        ));
    }
...
function esgi_widgets_init()
{
    register_sidebar(array(
        ...
        'description'   => __('Widgets ajoutés ici apparaîtront dans le footer.', 'ESGI-theme'),
